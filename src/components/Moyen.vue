...
import { enigmesMoyen } from '@/data/enigmesMoyen';
...
const availableCards = computed(() => {
    return enigmesMoyen.filter(card => !cardsStore.isEnigmeCardSolved(card.name));
});
...
const stats = computed(() => ({
    totalCards: enigmesMoyen.length,
...
onMounted(async () => {
    ...
    cardsStore.initializeStats([], enigmesMoyen);
    await nextTick();
});
