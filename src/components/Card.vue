...
// Vérifier si la carte était déjà résolue
onMounted(() => {
  if (cardsStore.isAnimeCardSolved(props.image.name)) {
    isCorrect.value = true;
    userInput.value = props.image.name;
  }
});
...
const checkAnswer = () => {
  ...
  if (userInput.value.toLowerCase() === props.image.name.toLowerCase()) {
    isCorrect.value = true;
    // Marquer la carte comme résolue
    cardsStore.markAnimeCardAsSolved(props.image.name, props.image);
  } else {
